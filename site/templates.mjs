...
export function about() {
  ...
  const site = SITE.website.replace(/^https?:\/\//, '').replace(/\/$/, '')
  const contact = `<section class="about-chapter about-perspective" data-motion="sectionHead"><header class="about-chapter-head"><div><p class="about-chapter-index">04 / Contact</p><h2>Find me around the web.</h2></div></header><p class="about-prose">Email <a href="mailto:${esc(SITE.email)}" rel="noreferrer">${esc(SITE.email)}</a> — Website <a href="${esc(SITE.website)}" target="_blank" rel="noreferrer">${esc(site)}</a> — GitHub <a href="${esc(SITE.github)}" target="_blank" rel="noreferrer">${esc(SITE.github.split('/').pop())}</a></p><p class="about-prose">${SOCIALS.map((s) => `<a href="${esc(s.url)}" target="_blank" rel="noreferrer">${esc(s.label)}</a>`).join(' · ')}</p></section>`
